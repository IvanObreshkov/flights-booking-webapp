...
class TestFlightsRoute:
    ...
    def test_flights_route__get_flight__flight_exists(self, mocker):
        ...
        mock_flights.return_value.get.return_value = mocked_flight

        flight_number = 5
        response, status_code = get_flight(flight_number=flight_number)

        mock_flights.return_value.get.assert_called_once_with(flight_number)

        assert {"Flight": mocked_flight.to_json()} == response
